...
description: "기획안 및 페르소나 소스를 분석하여 image.png 스타일의 브랜드 매니페스토 및 4대 자부심 지표를 최상단에 배치하고, 핵심 3대 시그니처 상품 단독 집중 조명과 하단 클라이언트 맞춤 20~35개(평균 28개) 상품 통합 쇼케이스를 완비한 6대 대단원 완전체 가상클라이언트를 생성하며, 브랜드 관련 레퍼런스(엄선된 정예 웰니스 롤모델 DB) [메뉴, 콘텐츠, 기능] 3대 축 교차 벤치마킹 후, 4단계 UI/UX 설계 전체(단일 롱스크롤 평균 28개 상품 인터랙티브 와이어프레임 HTML, 14개 전 화면 명세, 14개 전 섹션 스토리보드, 1200x1350/1680x2220 대형 풀 벡터 SVG)를 원스톱 자동 생성하는 마스터 통합 자동화 파이프라인 스킬이다."
argument-hint: "[가상클라이언트 번호 (예: 1, 2, 7, all)]"
---

# 🚀 통합 자동화 마스터 파이프라인 스킬 (Integrated Master Automation Pipeline)

본 스킬은 `C:\yyw\` 워크스페이스의 모든 소스 데이터를 기반으로, **[가상클라이언트 완전체 생성] ➔ [분석 스킬 발동] ➔ [브랜드 관련 레퍼런스 3대 축 정밀 사이트 분석] ➔ [1. 사이트 구조맵] ➔ [2. 서비스 흐름도] ➔ [3. 화면 설계서] ➔ [4. 메인페이지 스토리보드]**의 총 7단계 엔드-투-엔드(End-to-End) 프로세스를 최고 품질로 완결하는 통합 자동화 스킬입니다.
...
### 원칙 8: 고정 디렉터리 엄수 (새 폴더 생성 금지)
- 모든 설계 산출물은 `가상클라이언트 설계 결과\설계 출력물\` 하위의 **기존 4개 폴더(`사이트 구조맵`, `서비스 흐름도`, `화면 설계서`, `스토리보드 결과`)** 내부에만 저장합니다.
